...
//#define REQUIRED_APPROACH_SPEED (PSI_DOT * (LASER_RANGE - DES_DIST - CX10_SAFETY_SPEED * YAW_INPUT_DELAY))
...
//	rc_commands.rc_t = 0.;
	...
	init_butterworth_2_low_pass(&butter_left_reading, 2*0.2617,
			CX10_LASER_EMULATION_PERIODIC_PERIOD, left_init_distance);
	init_butterworth_2_low_pass(&butter_front_reading, 2*0.2617,
			CX10_LASER_EMULATION_PERIODIC_PERIOD, front_init_distance);
	init_butterworth_2_low_pass(&butter_right_reading, 2*0.2617,
			CX10_LASER_EMULATION_PERIODIC_PERIOD, right_init_distance);
